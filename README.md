...
| M4 — Cache & engine facade | ✅ Done |
| M5 — FFI & binary wire format | ⬜ Pending |
...
- **FFI-first / language-agnostic.** All public data types are `#[repr(C)]`
  ...
  (`include/urbix.h`) is intended to be auto-generated via `cbindgen`.
...
The library is in early development (currently `0.4.0`); the stable public API
is still taking shape as milestones land. `examples/cli_demo.rs` is a stub
skeleton for the walk-grid demo, to be filled in alongside the engine API, and
`Urbix_Project.md` §2 covers the module map and wire format.
