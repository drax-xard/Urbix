...
```c
// include/urbix.h (generated by cbindgen via build.rs)
typedef struct {
...
### Milestone 5 — FFI & Binary Format — ✅ DONE
...
| `include/urbix.h` | Generated C header, checked into the repo (`cargo build` regenerates it). |
| `cbindgen.toml` | Export allow-list, `ZONE_COUNT`/`CellFlags`/`InteriorId` mapping, and trailer `_Static_assert`s. |

**Memory contract:**
- `urbix_generate_chunk` allocates a buffer; caller must call `urbix_chunk_free`
  on the returned `UrbixChunkBuffer` to release it. The buffer is handed over by
  value as `{ data, len }`; only Rust's allocator frees it (via
  `urbix_chunk_free`), never a foreign `free`.
- Engine handle is opaque; only accessed through FFI functions.

**Tests:**
- C link-and-run integration test: `examples/basic_usage.c` compiles with `cc`,
  links against the crate's `staticlib`, generates a chunk, reads the header and
  cells, validates the buffer length, and frees it (`tests/c_link_run.rs`).
- Compile-only check of the generated header against the C consumer, with
  `_Static_assert`s on the 32/40-byte layouts (`tests/ffi_basic.rs`).
- Fuzz test: a stochastic sequence of create → generate → free → destroy
  through the raw FFI, run under the normal Rust allocator so double-free,
  use-after-free, and leaks are caught (`tests/ffi_basic.rs`).

**Exit criteria:** C example compiles and runs, no memory errors — met.

> Note: `urbix_get_zone` takes `double` (f64) world coordinates, matching the
> engine's `get_zone_affinity` signature, rather than the older `float` sketch.
